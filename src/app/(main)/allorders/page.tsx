...
export default function Orders() {
  ...
  async function handelOrders() {
    ...
    console.log("Orders data:", data) // 👀 Debug your API response
    setItem(Array.isArray(data) ? data : []) // ✅ guarantee an array
  }
      ...
      <div className="p-2.5 w-full">
        {item.length === 0 ? (
          ...
          item.map((order, index) => (
            <div
              key={index}
              className="border p-2.5 flex flex-col mb-4 space-y-8 w-full"
            >
              <div className="border my-8 p-2.5 rounded-2xl space-y-1 w-full">
                <p>Delivery status: {String(order.isDelivered)}</p>
                <p>Payment method: {order.paymentMethodType}</p>
                <p>Payment status: {String(order.isPaid)}</p>
              ...
              {Array.isArray(order.cartItems) && order.cartItems.length > 0 ? (
                order.cartItems.map((element) => (
                  <div
                    key={element._id}
                    className="border p-8 flex items-center justify-between rounded-2xl"
                  >
                    <Image
